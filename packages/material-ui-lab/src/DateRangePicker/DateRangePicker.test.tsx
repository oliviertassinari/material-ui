...
describe('<DateRangePicker />', () => {
  ...
  it('selects the range from the next month', () => {
    ...
    expect(
      onChangeMock.calledWith([
        new Date('2019-01-01T00:00:00.000'),
        new Date('2019-03-19T00:00:00.000'),
      ]),
    ).to.equal(true);
  });
  ...
  it('continues start selection if selected "end" date is before start', () => {
    ...
    expect(
      onChangeMock.calledWith([
        new Date('2019-01-19T00:00:00.000'),
        new Date('2019-01-30T00:00:00.000'),
      ]),
    ).to.equal(true);
  });

  it('starts selection from end if end text field was focused', () => {
    const onChangeMock = spy();
    render(
      <DesktopDateRangePicker
        renderInput={defaultRangeRenderInput}
  ...
  // Todo figure out why this failing
